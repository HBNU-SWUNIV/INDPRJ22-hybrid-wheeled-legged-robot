...
void setup() {
  ...
  error = ps2x.config_gamepad(49, 47, 48, 46, false, false); //setup pins and settings:  GamePad(clock, command, attention, data, Pressures?, Rumble?) check for error
  ...
  */
  ...
  //read controller
    ...
    if (ps2x.Button(PSB_PAD_UP)) {        //will be TRUE as long as button is pressed
      ...
      if ((ps2x.ButtonPressed(PSB_R2)) && (ps2x.ButtonPressed(PSB_L2))) {
        stopMotor();
        straightForwardGait();
    ...
    else if ((ps2x.ButtonPressed(PSB_R2)) && (ps2x.ButtonPressed(PSB_L2)) && (ps2x.Button(PSB_BLUE))) {
      stopMotor();
      straightForwardGait();
      delay(2000);
      turnLeftGait();
...
void calibServo() {
  ...
  RF.write(50 + RFcyl); // min 45 at 0 mm  -  max 125 at 100mm
...
void setServoRightRot() {
  ...
  RFP.write(90 - 35); // min 45 at 0 mm  -  max 125 at 100mm
...
void straightForwardGait()
{
  ...
  for (i = 1; i < 31; i++)  {
    RFP.write(75 + i);
    if (i < 16) {
      RF.write(105 - 4 * i + RFcyl );
      stopMotor();
    }
    else
    {
      RF.write(45 + 4 * (i - 15 ) + RFcyl);
      stopMotor();
    }
    delay(stepRun );
    // Serial.println(i);
